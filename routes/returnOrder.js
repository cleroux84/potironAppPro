...
const { createLabel, getShippingPrice, calculateTotalShippingCost, getDeliveryCostsPackages, returnedItemsByShipment } = require('../services/API/colissimo');
  ...
  try {
    ...
    if(returnAllOrder) {
      ...
      if(productSkuCalc.length === 1 && productSkuCalc[0].quantity === 1)  {
        ...
        const groupedItems = returnedItemsByShipment(shipments, filteredItems, quantitiesByRefs);
        priceByWeight = await getDeliveryCostsPackages(groupedItems, shipments);
        totalRefund = totalAsset - priceByWeight;
      }
    }
    ...
    if(returnAll) {
      ...
      if(productSku.length === 1) {
        if(productSku[0].quantity === 1) {
          ...
          const groupedItemsByShipment = returnedItemsByShipment(shipments, filteredItems, returnQuantities);
